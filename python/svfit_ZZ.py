...
class SVFitProducer(JetLepMetModule):
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        ...
        self.out.branch('Ztt_svfit_pt%s' % self.systs, 'F')
        self.out.branch('Ztt_svfit_eta%s' % self.systs, 'F')
        self.out.branch('Ztt_svfit_phi%s' % self.systs, 'F')
        self.out.branch('Ztt_svfit_mass%s' % self.systs, 'F')
        pass
    ...
    def analyze(self, event):
        ...
        self.out.fillBranch("Ztt_svfit_pt%s" % self.systs, result[0])
        self.out.fillBranch("Ztt_svfit_eta%s" % self.systs, result[1])
        self.out.fillBranch("Ztt_svfit_phi%s" % self.systs, result[2])
        self.out.fillBranch("Ztt_svfit_mass%s" % self.systs, result[3])
        return True
...
class SVFitRDFProducer(JetLepMetSyst):
    ...
    def run(self, df):
        branches = ["Ztt_svfit_pt%s" % self.systs, "Ztt_svfit_eta%s" % self.systs,
            "Ztt_svfit_phi%s" % self.systs, "Ztt_svfit_mass%s" % self.systs]
        all_branches = df.GetColumnNames()
        if branches[0] in all_branches:
            return df, []

        df = df.Define("svfit_zz_result%s" % self.systs,
            ...
            "Ztt_svfit_pt%s" % self.systs, "svfit_zz_result%s[0]" % self.systs).Define(
            "Ztt_svfit_eta%s" % self.systs, "svfit_zz_result%s[1]" % self.systs).Define(
            "Ztt_svfit_phi%s" % self.systs, "svfit_zz_result%s[2]" % self.systs).Define(
            "Ztt_svfit_mass%s" % self.systs, "svfit_zz_result%s[3]" % self.systs)
        return df, branches
